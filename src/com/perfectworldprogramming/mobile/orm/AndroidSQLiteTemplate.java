 ...
 * Either approach, the CursorAdapter will close the Cursor after it has completed conversion. No need to close Cursors yourself.
 * 
 * User: Mark Spritzler
 * Date: 3/10/11
 * Time: 10:07 PM
...
public class AndroidSQLiteTemplate implements JdbcOperations {
    ...
    public long insert(Object object) {
        ...
        if (id != -1) {
        	...
        	throw new DataAccessException("Unable to insert new record for object: " + object);
        }
        return id;
    }
    ...
    public long update(Object object) {
        ContentValues values = domainClassAnalyzer.createContentValues(object);
        String whereClause = domainClassAnalyzer.getPrimaryKeyFieldName(object.getClass()) + " = ?";
        String[] whereArgs = {String.valueOf(domainClassAnalyzer.getIdFromObject(object))};

        return sqLiteDatabase.update(object.getClass().getSimpleName(),
    ...
    public long delete(Object object) {
    	if (object == null) {
    		throw new DataAccessException("Please supply a non null domain object to delete(Object) method");
    	}
        String whereClause = domainClassAnalyzer.getPrimaryKeyFieldName(object.getClass()) + " = ?";
        String[] whereArgs = {String.valueOf(domainClassAnalyzer.getIdFromObject(object))};
        long results = -1;
        try {
        	...
        	if (results == 0) {
        		throw new DataAccessException("unable to delete " + object.getClass().getName() + ". Most likely the Primary Key id value does not exist in the database.");
        	}
    ...
    public long delete(String table, String columnName, String columnValue) {
    	if (table == null || columnName == null || columnValue == null || "".equals(table)|| "".equals(columnName)|| "".equals(table)) {
    		throw new DataAccessException("Please supply tableName, columnName and columnValue in order to delete any data");
    	}
    	String[] args = {columnValue};
    	long results = -1;
    	try {
    		results = sqLiteDatabase.delete(table, "" + columnName + "=?", args);
    	} catch (SQLiteException sle) {
    		throw new DataAccessException("Call to delete(String, String, String) failed. Please check you are passing in correct tableName, columnName and value to this method");
    	}
    	return results;
    }
    ...
    public long queryForLong(String sql, Object... args) {
        ...
        try {
        	...
        	throw new NoRowsReturnedException("Query For Long/Int did not return any rows for sql: " + sql);
        } finally {
        	if (statement != null) {
        		statement.close();
        	}
    ...
    public String queryForString(String sql, Object... args) {
        ...
        try {
        	...
        	throw new NoRowsReturnedException("Query For String did not return any rows for sql: " + sql);
...
//    }
    ...
    public <T> T queryForObject(String sql, Class<T> clazz, Object... args) {
        ...
        if (t == null) {
        	throw new DataAccessException("QueryForObject returned no results for query: " + sql);
        }
        return t;
    }

    @Override
    public <T> T queryForObject(String sql, CursorExtractor<T> cursorExtractor, Object... args) {
...
//    }
    ...
    private String generateStatementById(Class<? extends Object> clazz, Long id) {
        ...
        selectStatement.append(domainClassAnalyzer.getPrimaryKeyFieldName(clazz));
    ...
    private String replaceParametersInStatement(String sql, Object... args) {
    	...
    	if (sql.contains("?")) {
			throw new DataAccessException("Not all Query parameters have been set, please set all values for query to execute");
		}
